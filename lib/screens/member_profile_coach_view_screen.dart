...
class _MemberProfileCoachViewScreenState
  ...
  Future<void> loadData() async {
    try {
      ...
      final dashboardData = await MasterDataProvider.instance.fetchMemberData(
        memberId,
        force: false,
      );
      ...
      // ✅ The coach member-list RPC doesn't return weight_kg/height_cm
      // (kept lean for egress). Backfill them here from the full profile
      // fetch so DietBuilderScreen/CoachDietPreviewScreen — which both
      // receive this same widget.member map — can show Weight/Height.
      widget.member['weight_kg'] ??= profile?['weight_kg'];
