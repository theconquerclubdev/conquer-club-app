...
class _MemberPaymentSheetState extends State<MemberPaymentSheet> {
  ...
  Future<void> _processPayment() async {
    ...
    try {
      ...
      // Membership dates are calculated here so admin can see what WILL
      // apply once verified, but they are NOT written to the member's
      // profile yet — that only happens after admin approval.
      ...
      await Supabase.instance.client.from('payments').insert({
        ...
        'end_date': endDateStr,
        'is_manual': false, // Member-initiated
        'pricing_type': selectedPricingType,
      });

      // Membership is intentionally NOT updated here.
      // profiles.membership_start_date / end_date are only set once an
      // admin verifies the payment (see AdminPaymentsTab._verifyPayment).
